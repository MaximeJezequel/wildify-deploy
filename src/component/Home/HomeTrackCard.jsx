...
const HomeTrackCard = ({ topTrack }) => {
	...
	useEffect(() => {
		const getTopTrackImg = () =>
			axios
				.get(
					`https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=${process.env.REACT_APP_API_KEY}&artist=${topTrack.artist.name}&track=${topTrack.name}&format=json`
				)
